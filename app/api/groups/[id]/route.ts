...
export async function DELETE(
  ...
  try {
    ...
    // Clear group_id from any queue entries first (allows deletion even if in queue)
    const { error: queueClearError } = await supabaseServer
      .from('queue')
      .update({ group_id: null })
      .eq('group_id', id);

    if (queueClearError) {
      console.error('Error clearing group_id from queue:', queueClearError);

...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Waitlist API] Testing Supabase connection...');
    const { data: testData, error: testError } = await supabaseServer
      .from('queue')
      .select('id')
      .limit(1);
    
    console.log('[Waitlist API] Connection test:', { data: testData, error: testError });
    
    if (testError) {
      console.error('[Waitlist API] Connection test failed:', testError);
      return NextResponse.json({ error: `Database connection failed: ${testError.message}` }, { status: 500 });
    }

    const { queue_entry_ids, group_id } = await request.json();
    ...
    console.log('[Waitlist API] Fetching entries for IDs:', queue_entry_ids);
    const { data: allEntries, error: fetchError } = await supabaseServer
      .from('queue')
      .select('*')
      .in('id', queue_entry_ids);

    console.log('[Waitlist API] Fetch result:', { data: allEntries, error: fetchError });
    ...
    // Validate that all entries exist and are in 'waiting' status
    const invalidEntries = allEntries?.filter((entry: any) => entry.status !== 'waiting');
    if (invalidEntries && invalidEntries.length > 0) {
      return NextResponse.json({ 
        error: 'Some entries are not eligible for waitlist (must be in waiting status)',
        invalidEntries 
      }, { status: 400 });
    }

    // Update the specified entries to waitlist status
    ...
    return NextResponse.json({
      success: true,
      message: `Moved ${updatedEntries?.length || 0} entries to waitlist`,
      updatedEntries
    });

  } catch (error) {
    console.error('[Waitlist API] Full error details:', {
      error,
